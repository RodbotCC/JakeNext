...
### 2026-04-15 ~01:34 EDT — Chooser scripts reconciled to consolidated topology

**Action**: Updated the shared filesystem helper, module-progress refresh, and hourly chooser script so they resolve the workspace root correctly and use `signals/`, `capabilities/`, and `collaboration/` paths after directory consolidation.

**Outcome**: The hourly chooser can reuse the live Jake packet without creating ghost top-level `handoff/` or `jake/` directories.
...
**Action**: Updated `daily_substrate_sweep.mjs` so generated packets reference `ledgers/FDL.md`, `ledgers/FCLl.md`, `ledgers/MACRO_LEDGER.md`, and `ledgers/TCLl.md` instead of the old root-level paths.
...
### 2026-04-15 ~01:32 EDT — Safe worker paths repaired after capabilities consolidation

**Action**: Updated `lib/oracle_fs.mjs` to resolve the workspace root from the new `capabilities/scripts/lib/` location, corrected safe-worker queue paths to `collaboration/handoff/codex/`, and corrected safe action script invocations to `capabilities/scripts/`. Also updated direct helper references in `daily_substrate_sweep.mjs`, `daily_sylvia_reflection.mjs`, `create_event_packet.mjs`, and `route_event.mjs` so safe worker actions do not call stale top-level `scripts/`, `events/`, or `handoff/` paths.
