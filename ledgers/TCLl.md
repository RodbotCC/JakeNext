...
**Corrections**:
- Local continuity ledgers are `TCL.md`, not `ledgers/TCLl.md`
- Trigger rules and sweep outputs now point at `ledgers/FCLl.md` and `ledgers/TCLl.md` correctly
...
**Action**: Added the first real Ratio Lattice Ledger layer to JakeNext: a global `ledgers/RLLl.md`, local `RLL.md` files for key directories, a scored baseline over the current meaningful corpus, comparator law, and an orchestrator UI surface for the lattice.

**Files**:
- Added `ledgers/RLLl.md`
- Added `source/RLL.md`, `analysis/RLL.md`, `canon/RLLca.md`, `skills/RLL.md`, and `orchestrator/RLL.md`
...
### 2026-04-15 ~01:32 EDT — Codex safe worker repaired and found no eligible auto packet

**Action**: Re-read the live Codex inbox queue and `chooser/NEXT_STEP.md`, found the safe-worker script still pointed at pre-consolidation paths, repaired the local substrate path assumptions, then ran `node capabilities/scripts/consume_codex_safe_packets.mjs`.

**Files**:
- Modified `capabilities/scripts/lib/oracle_fs.mjs`
- Modified `capabilities/scripts/consume_codex_safe_packets.mjs`
- Modified `capabilities/scripts/create_event_packet.mjs`
- Modified `capabilities/scripts/daily_substrate_sweep.mjs`
- Modified `capabilities/scripts/daily_sylvia_reflection.mjs`
- Modified `capabilities/scripts/route_event.mjs`
- Updated `collaboration/handoff/codex/inbox/TCLclhcoi.md`
- Updated `capabilities/scripts/TCLcpsc.md`
- Updated `capabilities/TCLcp.md`

**Signals**:
- Safe worker result: `consumed: 0`, `skipped: 5`

**Corrections**:
- Safe-worker execution now uses `collaboration/handoff/codex/` and `capabilities/scripts/` after directory consolidation.
- Event packet helpers now use `signals/events/` and `collaboration/handoff/` after directory consolidation.

**Outcome**:
- Codex queue state is unchanged.
- `chooser/NEXT_STEP.md` remains truthful and still points at the active Jake-lane packet.
- Future explicit `codex_safe_auto` packets can be scanned by the repaired worker instead of failing on stale paths.

**Notes**:
- The live Codex inbox contains standard work orders, one `queue_only` packet, and local ledger files; none are eligible for autonomous consumption.
...
- Updated `chooser/runs/FCLchr.md`
- Updated `chooser/runs/TCLchr.md`
- Updated `chooser/TCLch.md`
...
**Files updated**: `orchestrator/index.html`, `orchestrator/styles.css`, `orchestrator/app.js`, `orchestrator/FCL.md`, `orchestrator/TCL.md`, `scripts/FCL.md`, `scripts/TCL.md`, `ledgers/MACRO_LEDGER.md`, `ledgers/TCLl.md`, `ledgers/RLLl.md`
...
- `ledgers/FDL.md`, `ledgers/TCLl.md`, `ledgers/MACRO_LEDGER.md`, `ledgers/RLLl.md`
...
- Updated `canon/FCLca.md` and `canon/TCLca.md`
...
- `ledgers/RLLl.md` updated with new skill corpus entries
...
- `northstar/FCLn.md`, `TCL.md`, `RLL.md`, `NSL.md` — full four-ledger discipline
...
**Files updated**:
- `identity/FCLi.md` — removed moved files, updated description
- `identity/TCLi.md` — logged the move
- `identity/NSLi.md` — updated to reflect pure-identity scope
- `ledgers/FDL.md` — added `northstar/` to topology (9 core directories)
- `ledgers/NSLl.md` — added `northstar/` to global alignment map at Critical tier
...
- `chooser/runs/FCLchr.md`
- `chooser/runs/TCLchr.md`
- `chooser/TCLch.md`
...
**Files updated**:
- `identity/FCLi.md`, `identity/TCLi.md`
- `ledgers/FCLl.md`
...
- `ledgers/FCLl.md` — added NSL to ledgers directory
...
- `ledgers/FCLl.md` — added claude-code/ to global contents
- `ledgers/TCLl.md` (this entry)
...
**Files created**:
- `signals/FCLs.md`, `signals/TCLs.md`
- `collaboration/FCLcl.md`, `collaboration/TCLcl.md`
- `capabilities/FCLcp.md`, `capabilities/TCLcp.md`
- `knowledge/FCLk.md`, `knowledge/TCLk.md`

**Files updated**:
- `ledgers/FDL.md` - updated topology map with parent directories
- `ledgers/FCLl.md` - updated root contents
- `ledgers/TCLl.md` (this entry)
...
### 2026-04-15 ~01:33 EDT — Sylvia hourly chooser advanced
...
**Files created**:
- `Framed/FCL.md` — local contents ledger for the Framed app directory
- `Framed/TCL.md` — local continuity ledger for the Framed directory

**Files updated**:
- `ledgers/FDL.md` — added Framed/ to topology map under new "Product Directories" section
- `ledgers/FCLl.md` — added Framed/ to root directory contents
- `ledgers/MACRO_LEDGER.md` — added Framed to status table, navigation, workspace tree
- `ledgers/TCLl.md` — this entry
- `ledgers/NSLl.md` — added Framed to global alignment map
- `ledgers/RLLl.md` — added Framed corpus entry with baseline vector

**Context**: Framed is a visual operating-manual system and glue layer for persistent autonomy. It was previously standalone at `/Users/jakeaaron/Documents/Framed/` with no version control. It has live capabilities: block builder with canvas-first drawer layout, flow builder with drag connections, manual view (topological-order slides), command bridge (API for agent orchestration), block relationship graph (IndexedDB v2), annotation scale controls, settings page, and statistics.

**Corrections**:
- Framed is now under JakeNext version control and ledger discipline
- Future Framed sessions must update local and global ledgers per the four-ledger protocol

**Outcome**:
- The Framed product is now visible, indexed, and governed within the JakeNext workspace
- All four global ledger types have been updated
- Framed has its own local FCL and TCL for directory-level continuity

**Notes**:
- Immediate Framed work priorities: finish contextual editing, reduce UI clutter, strengthen app indexing
- Framed's node_modules and dist should be .gitignored before first push

### 2026-04-16 ~12:00 EDT — Barcode rename: 145 ledger files renamed, 74+ cross-references updated

**Action**: Renamed all FCL.md, TCL.md, RLL.md, and NSL.md files across the workspace to include a directory-path barcode suffix. Updated cross-references in 74+ files. Updated 4 scripts for barcode-aware ledger detection.

**Barcode convention**:
- Suffix = first letter(s) of each directory segment from root
- Top-level 'c' dirs use 2 letters: ca=canon, cp=capabilities, ch=chooser, cl=collaboration
- Sibling overrides: sc=scripts, sk=skills, co=codex, cc=claude-cowork
- Skill dirs (hyphenated): first letter of each word
- Example: `collaboration/handoff/codex/done/TCL.md` → `TCLclhcod.md`

**Files renamed**: 145 ledger files  
**Cross-references updated**: 74+ files  
**Scripts updated**: dispatcher.mjs, consume_codex_safe_packets.mjs, orchestrator_server.mjs, detect_ledger_drift.mjs  
**NOT renamed**: FDL.md, MACRO_LEDGER.md, MACRO_RLL.md (global unique files)

**Outcome**: Every ledger file now has a unique filename encoding its location. `ledgerBarcode()` function added to oracle_fs.mjs for runtime barcode computation.

### 2026-04-16 ~11:30 EDT — Alignment sweep: breakthrough/, clips/ deletion, ONAL move, stale FCL fix

**Action**: Full alignment sweep of the JakeNext directory. Created `breakthrough/` for theoretical frameworks not yet bedrock. Moved `ONAL_PHILOSOPHY.md` from canon/ to breakthrough/. Deleted `clips/` (empty stub directory — 4 .txt files with no real content). Rewrote `ledgers/FCLl.md` to fix 11 stale references to pre-consolidation directory names.

**Files created**:
- `breakthrough/FCLb.md`, `breakthrough/TCLb.md`, `breakthrough/RLLb.md`, `breakthrough/NSLb.md`

**Files moved**:
- `canon/ONAL_PHILOSOPHY.md` → `breakthrough/ONAL_PHILOSOPHY.md`

**Files deleted**:
- `clips/` (entire directory — FCL.txt, TCL.txt, RLL.txt, NSL.txt stubs)

**Files updated**:
- `ledgers/FCLl.md` — fully rewritten to reflect post-consolidation topology
- `ledgers/FDL.md` — added breakthrough/ to topology
- `ledgers/MACRO_LEDGER.md` — added breakthrough to status, navigation, workspace tree
- `ledgers/NSLl.md` — added breakthrough to alignment map
- `canon/FCLca.md` — marked ONAL as moved

**Corrections**:
- `ledgers/FCLl.md` was referencing 11 pre-consolidation directory names (jake/, skills/, events/, etc.). Now reflects the actual post-consolidation structure (collaboration/jake/, capabilities/skills/, signals/events/, etc.)
- ONAL and delta physics content are now explicitly separated from canonical doctrine — confident but not bedrock

**Outcome**: Directory structure is cleaner and ledger truth matches filesystem truth.

### 2026-04-16 ~11:00 EDT — Pieces MCP integration and Claude memory symlink installed

**Action**: Integrated two external memory/activity systems into JakeNext with full four-ledger discipline.

**Part 1 — Pieces Behavioral Memory:**
- Created `pieces/` top-level directory with FCL, TCL, RLL, NSL, and SWEEP_POLICY.md
- Created `pieces/sweeps/` and `pieces/patterns/` subdirectories with FCL + TCL each
- Created `capabilities/skills/claude-cowork-pieces-ingress/` skill (SKILL.md, FCL, TCL) following the Slack ingress pattern
- Updated `capabilities/scripts/lib/oracle_fs.mjs` — added `pieces_behavioral` path classification and `claude-cowork` routing
- Updated `signals/triggers/TRIGGER_RULES.md` — added 3 Pieces event types and path routing rule
- Updated `capabilities/skills/FCLcpsk.md` — registered new skill

**Part 2 — Claude Auto-Memory:**
- Created symlink `claude-memory` → `~/.claude/projects/-Users-jakeaaron-JakeNext/memory/`
- Added `claude-memory` to `.gitignore`
- Treated as read-only reference entry in global ledgers (no local ledger files inside symlink)

**Files created**:
- `pieces/FCLp.md`, `pieces/TCLp.md`, `pieces/RLLp.md`, `pieces/NSLp.md`, `pieces/SWEEP_POLICY.md`
- `pieces/sweeps/FCLps.md`, `pieces/sweeps/TCLps.md`
- `pieces/patterns/FCLpp.md`, `pieces/patterns/TCLpp.md`
- `capabilities/skills/claude-cowork-pieces-ingress/SKILL.md`, `FCL.md`, `TCL.md`
- `claude-memory` (symlink)

**Files updated**:
- `ledgers/FDL.md` — topology: pieces/, claude-memory, pieces ingress skill
- `ledgers/FCLl.md` — root contents: pieces/, claude-memory/
- `ledgers/MACRO_LEDGER.md` — status, navigation, workspace tree
- `ledgers/NSLl.md` — pieces/ = High, claude-memory/ = Low
- `ledgers/RLLl.md` — pieces/SWEEP_POLICY.md baseline vector added
- `capabilities/scripts/lib/oracle_fs.mjs` — path classification + routing
- `signals/triggers/TRIGGER_RULES.md` — event types + path routing
- `capabilities/skills/FCLcpsk.md` — new skill registered
- `.gitignore` — claude-memory exclusion

**Outcome**:
- Pieces MCP is now a governed signal source with sweep policy, ingress skill, and full ledger discipline
- Claude auto-memory is visible inside JakeNext as read-only orientation context
- Ready for first live sweep and scheduling
